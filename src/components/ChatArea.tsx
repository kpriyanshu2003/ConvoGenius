    ...
    createInteraction({ title: message, content: "Lorem Ipsum" })
      .then((data) => {
        if (params) {
          ...
          createCollective(localStorage.getItem("user"), data?.id)
            .then((res) => localStorage.setItem("collective", res?.id ?? ""))
            .then(() => router.push(`/${data?.id}`))
            .catch((error) => console.log(error));
        }
      })
      .catch((error) => console.log(error));
      ...
      <div className="relative h-screen w-screen">
        {!params && (
          <div className="absolute -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
            <div className="mb-20 grid place-items-center">
              <div className="border w-12 px-2 aspect-square flex h-full items-center justify-center rounded-full bg-white text-gray-950">
                <svg
                  width="41"
                  height="41"
                  viewBox="0 0 41 41"
        ...
        <div className="flex w-full max-w-3xl items-end space-x-2 absolute bottom-10 left-1/2 -translate-x-1/2">
          <Textarea
            ...
            value={message}
            className={`px-5 py-3 border-[#424242] h-12 border-2 w-[100vh] resize-none overflow-auto scrollbar-hide`}
            onChange={(e) => {
          ...
          <Button
            ...
            className={`h-12 w-12 ${
              message.trim().length > 0
                ...
                : "cursor-default bg-[#383838]"
            }`}
          >
            <FaArrowUp className="text-[#1e1e1e] h-10 w-10" />
          </Button>
        </div>
        <div className="absolute bottom-3 text-[#b4b4b4] text-sm -translate-x-1/2 left-1/2">
